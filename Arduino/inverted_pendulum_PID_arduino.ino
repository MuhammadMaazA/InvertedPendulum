...
void loop() {
  ...
  if (overrideActive) {
    if (overrideState == 1 && (currentTime - overrideStartTime >= 4300)) {
      setpointAngle = -2.0;
      overrideState = 2;
      overrideStartTime = currentTime;
      Serial.println("Switching override: Target angle set to -2° for 2 seconds");
    } else if (overrideState == 2 && (currentTime - overrideStartTime >= 2500)) {
      setpointAngle = 0.0;
  ...
  if (currentTime - lastControlTime >= controlInterval) {
    ...
    integralError += theta * dt;
    float u = Kp * theta + Ki * integralError + Kd * dtheta;
